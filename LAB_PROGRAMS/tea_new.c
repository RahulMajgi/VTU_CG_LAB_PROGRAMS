...
void displaySolid(void)
{
	...
	glLightfv(GL_LIGHT0, GL_SHININESS, lightIntensity); //glLightfv(GL_LIGHT0, GL_DIFFUSE, lightIntensity);

	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	double winHt = 1.0;
	glOrtho(-winHt * 64 / 48.0, winHt * 64 / 48.0, -winHt, winHt, 0.1, 100.0);//-1,1,-1,1,0,100
	...
	wall(0.02); //floor
	glPushMatrix();
	glRotated(90.0, 0.0, 0.0, 1.0);    //draw left wall after rotating z axis by 90degre
	wall(0.02);
	glPopMatrix();

	glPushMatrix();
	glRotated(-90.0, 1.0, 0.0, 0.0);   //draw back wall
	wall(0.02);
	glPopMatrix();
	glFlush();
}
